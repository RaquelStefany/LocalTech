<?php
    ...
    if(!empty($dados['Cadastrar'])){
        ...
        if($lemial > 0){
            echo "<script>window.location.replace('../Login/login-tecnico.php');</script>";
            ...
            if($erro == 1){                
                ...
                if($pegar_id_tecnico > 0){
                    ...
                    if($cadastrar_data->rowCount()){
                        ...
                        $tecnico = $conexao->prepare("INSERT INTO contato (id_tecnico, codificacao, email, telefone, verificacao) VALUES ('" . $_SESSION['id_cadastro'] . "', '" . $codificacao . "', '" . $dados['email'] . "', '" . $dados['telefone'] . "', 'Sim')");
                        $tecnico->execute();
