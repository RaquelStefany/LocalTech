...
function verificaForcaSenha(){
	...
	{
		$('#password-status').html("<span style='color:red;margin-left: 10px;;font-size: larger;'>Fraco, insira no mínimo 8 caracteres</span>");
        $('#cadastrar').prop('disabled', true);
	} else {  	
		if($('#password').val().match(numeros) && $('#password').val().match(alfabeto) && $('#password').val().match(chEspeciais))
		{            
			$('#password-status').html("<span style='color:green;margin-left: 10px;;font-size: larger;'><b>Forte</b></span>");
            $('#cadastrar').prop('disabled', false);
		} else {
			$('#password-status').html("<span style='color:orange;margin-left: 10px;;font-size: larger;'>Médio, insira um caracter especial e um número</span>");
            $('#cadastrar').prop('disabled', true);
		}
	}
}